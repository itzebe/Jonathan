...
export async function POST(request: Request) {
  ...
  try {
    ...
    const result: ExecutionResult = {
      ...
      txHash: `0x${crypto.randomUUID().replaceAll('-', '')}${crypto.randomUUID().replaceAll('-', '').slice(0, 24)}`,
      bscScanUrl: isDemoMode ? 'https://bscscan.com/tx/' : `https://bscscan.com/tx/0x${crypto.randomUUID().replaceAll('-', '')}`,
